function construct_subplots_m(err_data, err_data_m, pr,savename,plot_method,save_data)
...
% Create plot
for i = 1:size(Y1,2)
    plot(pr.mspan1,Y1(1:length(pr.mspan1),i),'-s','DisplayName',['MoRAM,s=',num2str(pr.s_span(i))],'LineWidth',2);
end
for i = 1:size(Y2,2)
    plot(pr.mspan1,Y2(1:length(pr.mspan1),i),'--o','DisplayName',['MulShot,s=',num2str(pr.s_span(i))],'LineWidth',2);
end
...
%     'Interpreter','latex','FontSize',22,'xoff',0.0,'yoff',-0.1);
% end
savepath = ['./results/mod_recovery_results/mod_',savename,num2str(datenum(datetime('now')))];
if save_data == 1
    
    save([savepath,'.mat'],'err_data','err_data_m');
end
