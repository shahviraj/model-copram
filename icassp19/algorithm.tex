...
It is evident that if we can recover $\mathbf{p^*}$ successfully, we can calculate the true compressed measurements $\langle \mathbf{a_i} \cdot \mathbf{x^*} \rangle$ and use them to reconstruct $\mathbf{x^*}$ with any sparse recovery algorithm such as CoSaMP~\cite{needell2010cosamp} or basis-pursuit~\cite{chen2001atomic}.
 ...
 \begin{figure}[t]
	\begin{subfigure}[b]{0.63\linewidth}
		...
		\resizebox{\linewidth}{!}{
			...
			{        
				\coordinate (A\x) at ($(0,0)+(\x*2.5cm,0)$) {};
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}
		...
		%\State $\mathbf{{x}^{t+1}}\leftarrow \argmin_{\mb{u}=[\mathbf{x~d}]^\t}\norm{\mb{u}}_1~s.t.~ \begin{bmatrix} \mathbf{A} & \mathbf{I} \end{bmatrix}\mb{u} = \mathbf{y^t_c}$ 
...
In the next theorem, we provide guarantee for the AltMin based descent step of our algorithm. For simplicity, we limit our analysis of the descent to only one iteration of our Alternative Minimization based algorithm. In fact, as proven in our analysis, theoretically only one iteration of JP is required for exact signal recovery. Contrast to that, in practice we observe that our algorithm requires more than one alternative minimization iterations to converge to the optimum solution.
