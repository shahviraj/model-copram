...
In this analysis, our goal is to find out the total number of measurements ($N=\card(T)$), that can be corrected through the thresholding the measurements. Only these $N$ measurements would be used in the estimation step.
...
For the next lemma, we first introduce the concept of binary $\epsilon$-stable embedding as appeared in~\cite{Jacques2013}. We note that $\mathcal{B}^m$ is a Boolean cube defined as $\mathcal{B}^m := \{0,1\}^m$; and $S^{n-1}:=\{\mb{x}\in \R^n : \norm{\mb{x}}_2 = 1\}$ is the unit hyper-sphere of dimension $n$.
...
\begin{align}
m \geq \max & \Bigg[ C\left(\frac{t}{\epsilon}\right)^2s \left(1-2\frac{\phi(R-\rho)}{(R-\rho)} \right)^{-1}, \nonumber \\ 
& \frac{2}{\kappa^2}\left(s\log{(n)} + 2s\log{\left(\frac{35}{\kappa}\right)}+\log{\left(\frac{2}{\eta}\right)}\right)\Bigg]
\label{eq:ineq4}
\end{align}

Similarly for the case when $R<\rho$ we get,
\begin{align}
m \geq \max &  \Bigg[C\left(\frac{t}{\epsilon}\right)^2s \left(\frac{2R}{(1+R^2)} \phi(R) - \frac{\phi(\rho)}{\rho}\right)^{-1}, \nonumber \\ 
& \frac{2}{\kappa^2}\left(s\log{(n)} + 2s\log{\left(\frac{35}{\kappa}\right)}+\log{\left(\frac{2}{\eta}\right)}\right) \Bigg]
\label{eq:ineq5}
\end{align}