...
\subsubsection{Measurement correction step}\label{meascorr}In this step, based on the absolute value of compressed modulo measurements, we identify the measurements for which we can undo the effect of modulo operation by guessing the correct value of bin-index $\mathbf{p}$. We define the set $U$ as set of indices of all such measurements that can be corrected.

We determine the measurements to be corrected based on the method described in section~\ref{sec:init}. 
In this analysis, our goal is to find out the total number of measurements ($N=\card(U)$), that can be corrected through the thresholding of the measurements. Only these $N$ measurements would be used in the estimation step.
...
Being a linear combination of Gaussian random variables, $y_{c,i}$ also follows Gaussian distribution.
...
We calculate the initial estimate using the measurements in set $U$ as following,
$$
\mathbf{{x}^0} = H_s(M) = H_s\left(\frac{1}{\card(U)}\sum_{i\in U}y_{i}a_{i}\right).
$$
$$
\mb{x^0} = H_s\left( \frac{1}{N}\sum_{i=1}^{N}y_{U,i}a_{U,i}\right)
$$
We use the versions of $\mb{y}$ and $\mb{A}$ truncated to the row indices belonging to set $U$, and column indices to set $S$. We denote such submatrices with the subscript $U\times S$. 
...
\implies \norm{\frac{1}{N} \mb{A}_{\scriptscriptstyle{U\times S}}^\t \mb{A}_{\scriptscriptstyle{U\times S}} - I_s} &\leq \kappa \\
\end{align*}
