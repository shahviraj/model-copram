...
% 
% figure;
% plot(sig_span, err_init, sig_span, err_sig);
% 
...
for i = 1:length(perterb_prob)
    ...
    y_perter = y_p;
    y_perter(perterb_idx)= -y_perter(perterb_idx);
    
    x = cosamp((y.*y_perter)/sqrt(m), A/sqrt(m),s,10);
    
    err_y_per(i) = norm((y.*y_p)-(y.*y_perter))/norm((y.*y_p));
    err_x_per(i) = norm(x-z)/norm(z);
    
end
figure;
plot(perterb_prob,err_y_per);
title('Abs: Variation of relative reconstruction error in x with number of perturbations in p');
figure;
plot(perterb_prob, err_x_per);
title('Abs: Variation of relative error in y with number of perturbations in p');
